Ext.define('SearchTool.controller.QueryFilters', {
        ...
        getSourcesData: function (store, records, opts) {
            ...
            if (tmp && tmp.repoDefinitions && tmp.repoDefinitions.length > 0) {
                ...
                for (i = 0; i < this.data.items.length; i++) {
                    t = this.data.items[i];
                    chkgroup = Ext.create('Ext.form.CheckboxGroup', {
                            fieldLabel: t.data.repositoryId,
                            itemId: 'chkgrp' + t.data.repositoryId,
                            layout: 'vbox',
                            labelWidth: 75,
                            columns: [.8]
                        });
                ...
                pnl.add({
                        xtype: 'component',
                        html: 'No Sources available at this time'
                    });
            }
        },
        getAllResultsData: function (store, records, opts) {
            ...
            var btnExport = Ext.ComponentQuery.query('#btnExport')[0],
            ...
            if (tmp && tmp.searchId && tmp.searchId.length > 0) {
                btnExport.enable();
                Ext.ComponentQuery.query('#btnSearchId')[0].id = tmp.searchId;
                //TODO: change hrefs of each button to add tmpsearchID 
                //btnCsv.href = (urlExportRoot + urlExportTokenCsv + tmp.searchID; //only works on first and prior to render or ? so below ifs are needed
                //if (btnCsv.el)
                //     btnCsv.el.dom.children[0].href ...
            }
            if (tmp && tmp.searchFilters) {
                ...
                for (; i < searchFilters.length; i++) {
                    ...
                    j = Ext.create('SearchTool.model.QueryFilter', {
                            type: '',
                            key: t.filter,
                            operator: 'eq',
                            tip: t.value,
                    ...
                    case 'enddate':
                        j.data.type = 'enddate';
                        Ext.ComponentQuery.query('#dtUserSearchTo').setValue(t.value);
                        break;
                    default:
                        j.data.type = 'facet';
                    }
            ...
            if (tmp && tmp.coreFacets && tmp.coreFacets.length > 0) {
                ...
                pnl.add(Ext.create('Ext.panel.Panel', {
                            width: '100%',
                            border: false,
                            title: 'No Filters Available',
                            items: [{
                                    xtype: 'component',
                                    html: 'Filters are not available at this time'
                                }
                            ]
                        }));
        ...
        querySaveHandler: function (btn, e) {
            Ext.create('SearchTool.view.main.component.WinSave', {
                    title: 'Save Query'
                }).show();
        },
        queryRunHandler: function (btn, e) {
            var i = 0,
                arrSaved = btn.up('container').items.items;
            ...
            arrFilters.push(Ext.create('SearchTool.model.QueryFilter', {
                        type: 'savedSearch',
                        key: 'savedSearchId',
                        operator: 'eq',
                        value: btn.id
        ...
        searchHandler: function (btn, e) {
            ...
            if (form.getForm().isValid()) {
                ...
                var params = form.getValues(),
                    t = null,
                    filtersStore = this.getQueryFiltersStore(),
                    i = 0,
                    arrFilters,
                    arrParams = [{
                            type: 'searchkeyword',
                            key: 'keywordString',
                            value: params.keywordString.trim()
                        }, {
                            type: 'searchboolean',
                            key: 'booleanString',
                            value: params.txtSearchBoolean.trim()
                        }, {
                            type: 'datefield',
                            key: 'datefield',
                ...
                bbarFilters = Ext.ComponentQuery.query('#bbarFilters')[0];
                ...
                for (; i < arrParams.length; i++) {
                    ...
                    if (t.value && t.value != "") { //if value
                        t = Ext.create('SearchTool.model.QueryFilter', {
                                type: t.type,
                                key: t.key,
                                operator: 'eq',
                                tip: t.value,
                        ...
                        if (t.data.type == 'searchkeyword' || t.data.type == 'searchboolean')
                            bbarFilters.add({
                                    text: t.data.value,
                                    tooltip: t.data.key + '=' + t.data.value,
                                    cls: 'resultsgridbtn'
                                });
                    }
                }
        ...
        filterRelaxAll: function (b, e) {
            Ext.Msg.confirm('Confirm - Remove all filters', 'Are you sure you want to relax all filters?', function (btn) {
                console.log(btn);
                if (b.pressed) { //facetgroup class to table
                }
            });
        }
